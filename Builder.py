...
class Builder:
    ...
    def low_level_error_handling(self):
        ...
        except DriveValueException as drive_value_ex:
            try:
                return self.drive.foo_raises_drivevalue_exception(0)               # catching the exception & trying something else if we can
            except Exception as ex:
                raise BuilderException(f" Couldn't performe action {drive_value_ex} {ex}")   # the last exception should be generic to catch everything. We also propagated the previous drive_value_ex Exception
